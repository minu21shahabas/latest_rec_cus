...
def viewrecur(request,id):    
    ...
    comment =recur_comments.objects.filter(recom=id) 

    print(product.id)
    return render(request,'recur_invoice.html',{'customer':cust,'allproduct':items,'product':product,'itemstable':table,'company':company,'comment':comment})
...
def add_comment_cust(request,id):
    if request.method == 'POST':
        ...
        custcom = customer.objects.get(id=id)
        
        comment = request.POST['comments']
        user = User.objects.get(id=request.user.id)
        
        comments= cust_comment(custcom=custcom,user=user,comment=comment)
        comments.save()
        return redirect('view_one_customer',id)
...
def recurinvoice_customer(request):
    ...
    if request.method=='POST':
        cust_type = request.POST.get('itemtype')
        print(cust_type)
